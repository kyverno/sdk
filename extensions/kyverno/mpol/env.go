...
var mpolCompilerVersion = version.MajorMinor(1, 0)
...
func NewEnv() (*environment.EnvSet, *compiler.VariablesProvider, error) {
	...
	// ResolverEnvOption must come after declOptions: it wraps whatever CustomTypeProvider
	// declOptions installed, so that JSONPatch{} struct literals are resolved first before
	// delegating to the decl/variables provider chain.
	...
	extendedBase, err := base.Extend(
		...
		environment.VersionedOptions{
			...
			EnvOptions: []cel.EnvOption{
				globalcontext.Lib(nil, globalcontext.Latest()),
				http.Lib(http.Context{ContextInterface: http.NewHTTP()}, http.Latest()),
				resource.Lib(nil, "", resource.Latest()),
				image.Lib(image.Latest()),
				imagedata.Lib(nil, imagedata.Latest(), nil),
				user.Lib(user.Latest()),
				hash.Lib(hash.Latest()),
